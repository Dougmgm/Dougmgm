# 👋 Olá, eu sou Douglas Menchon

### Desenvolvedor de Software | C# / .NET

Sou Desenvolvedor de Software com **4+ anos de experiência profissional**, atualmente atuando na **Benner**, onde trabalho no desenvolvimento, manutenção e evolução de sistemas na área da saúde.

Minha principal especialidade está no ecossistema **C#/.NET e ASP.NET Core**, com experiência em aplicações desktop, regras de negócio, integrações, bancos de dados e qualidade de software, além de JavaScript e bancos de dados relacionais, com foco em Arquitetura de Software, APIs REST, performance e boas práticas de desenvolvimento.

Tenho interesse por **arquitetura de software, desenvolvimento backend, sistemas distribuídos e Cloud Computing**.

---

## 💻 Sobre mim

* 🔹 4+ anos de experiência profissional em desenvolvimento de software
* 🔹 Experiência sólida com **C# e .NET**
* 🔹 Desenvolvimento e manutenção de sistemas corporativos
* 🔹 Experiência com **WPF, XAML e MVVM**
* 🔹 Desenvolvimento e integração de **APIs REST e SOAP**
* 🔹 Experiência sólida com **Oracle e PL/SQL**
* 🔹 Experiência com **Entity Framework e NHibernate**
* 🔹 Testes automatizados utilizando **NUnit**
* 🔹 Aplicação de **SOLID, Clean Code e Design Patterns**
* 🔹 Experiência com sistemas legados e modernização incremental
* 🔹 Experiência em ambientes corporativos e sistemas críticos
* 🔹 Inglês avançado

---

## 🛠️ Tecnologias

### Backend

![C#](https://img.shields.io/badge/C%23-512BD4?style=flat-square\&logo=csharp\&logoColor=white)
![.NET](https://img.shields.io/badge/.NET-512BD4?style=flat-square\&logo=dotnet\&logoColor=white)
![ASP.NET](https://img.shields.io/badge/ASP.NET-512BD4?style=flat-square\&logo=dotnet\&logoColor=white)

### Desktop

![WPF](https://img.shields.io/badge/WPF-512BD4?style=flat-square\&logo=windows\&logoColor=white)
![XAML](https://img.shields.io/badge/XAML-0C54C2?style=flat-square\&logo=windows\&logoColor=white)
![MVVM](https://img.shields.io/badge/MVVM-Arquitetura-512BD4?style=flat-square)

### Bancos de Dados

![Oracle](https://img.shields.io/badge/Oracle-F80000?style=flat-square\&logo=oracle\&logoColor=white)
![PL/SQL](https://img.shields.io/badge/PL%2FSQL-F80000?style=flat-square\&logo=oracle\&logoColor=white)
![SQL](https://img.shields.io/badge/SQL-4479A1?style=flat-square)

### APIs e Integrações

![REST](https://img.shields.io/badge/REST-02569B?style=flat-square)
![SOAP](https://img.shields.io/badge/SOAP-6A1B9A?style=flat-square)
![JSON](https://img.shields.io/badge/JSON-000000?style=flat-square\&logo=json\&logoColor=white)
![XML](https://img.shields.io/badge/XML-FF6600?style=flat-square)

### Testes e Desenvolvimento

![NUnit](https://img.shields.io/badge/NUnit-25A162?style=flat-square\&logo=nunit\&logoColor=white)
![Git](https://img.shields.io/badge/Git-F05032?style=flat-square\&logo=git\&logoColor=white)
![GitHub](https://img.shields.io/badge/GitHub-181717?style=flat-square\&logo=github\&logoColor=white)

---

## 🏗️ Engenharia de Software

Busco desenvolver software que seja **manutenível, testável, legível e preparado para evoluir**.

Tenho experiência e interesse em:

* **SOLID**
* **Clean Code**
* **Design Patterns**
* **MVVM**
* **Arquitetura em Camadas**
* **Injeção de Dependência**
* **DTOs**
* **Repository Pattern**
* **Testes Unitários**

---

## 🚀 Atualmente estudando

Estou constantemente expandindo meus conhecimentos para além da minha stack principal.

* 🐇 **RabbitMQ e mensageria**
* 🅰️ **Angular**
* 🐳 **Docker**
* ☁️ **Cloud e sistemas distribuídos**
* 🏗️ **Arquitetura de Software**

---

## 🎯 Áreas de interesse

Tenho interesse principalmente em oportunidades e projetos envolvendo:

**C# • .NET • Backend • Full Stack • APIs • Arquitetura de Software • Sistemas Distribuídos • Sistemas Corporativos**

Também tenho interesse em trabalhar com **equipes internacionais**, contribuindo tecnicamente e evoluindo continuamente minhas habilidades.

---

## 🎓 Formação

**Análise e Desenvolvimento de Sistemas**
UniFatecie

**Ciências Econômicas**
Universidade Estadual de Maringá — UEM

---

## 📫 Vamos conversar?

[![LinkedIn](https://img.shields.io/badge/LinkedIn-Douglas_Menchon-0A66C2?style=flat-square\&logo=linkedin\&logoColor=white)](https://www.linkedin.com/in/douglas-menchon-8b96b7144/)

---

> *"Software de qualidade não é apenas código que funciona, mas código que pode ser compreendido, mantido e evoluído."*
